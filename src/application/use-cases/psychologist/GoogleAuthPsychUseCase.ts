...
export default class GoogleAuthPsychUseCase implements IGoogleAuthPsychUseCase {
  ...
  async execute(dto:PsychGoogleAuthDTO): Promise<LoginResponse> {
    ...
    const accessToken = generateAccessToken({id:psych.psychId!,role:Role.PSYCHOLOGIST});
    const refreshToken =generateRefreshToken({id:psych.psychId!,role:Role.PSYCHOLOGIST});  
