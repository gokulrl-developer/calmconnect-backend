...
export default class CreateSpecialDayUseCase implements ICreateSpecialDayUseCase{
    ...
    async execute(dto:CreateSpecialDayDTO){
      ...
      if(specialDay.type==="absent"){
        if(specialDay.startTime || specialDay.endTime || specialDay.durationInMins || specialDay.bufferTimeInMins){
        ...
        if(specialDay.type==="override"){
            ...
            if(!specialDay.durationInMins || specialDay.durationInMins<20 || specialDay.durationInMins>180){
                throw new AppError(ERROR_MESSAGES.DURATION_REQUIRED,AppErrorCodes.VALIDATION_ERROR)
            }
        }
      }
      const conflictingSpecialDay=await this._specialDayRepository.findActiveByDatePsych(dto.date,dto.psychId);
      if(conflictingSpecialDay){
        throw new AppError(ERROR_MESSAGES.CONFLICTING_SPECIAL_DAY,AppErrorCodes.CONFLICT)
      }
      if(specialDay.type==="override"){
      const conflictingQuickSlots=await this._quickSlotRepository.findOverlappingActiveByTimeRangePsych(new Date(specialDay.startTime!),new Date(specialDay.endTime!),specialDay.psychologist)
